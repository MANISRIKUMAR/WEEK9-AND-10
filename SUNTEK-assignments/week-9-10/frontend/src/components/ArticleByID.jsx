...
function ArticleByID() {
  ...
  useEffect(() => {
    const getArticle = async () => {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/common-api/articles/${id}`, { withCredentials: true });
  ...
  const addComment = async (commentObj) => {
    try {
      ...
      const res = await axios.put(import.meta.env.VITE_API_URL + "/user-api/articles", payload, { withCredentials: true });
      if (res.status === 200) {
        toast.success(res.data.message);
        setArticle(res.data.payload);
        reset({ comment: "" });
